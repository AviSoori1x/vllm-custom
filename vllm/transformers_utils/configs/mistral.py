...
def adapt_config_dict(config_dict: dict[str, Any],
    ...
    # Handle Omnistral case with both vision and audio
    if is_vision and is_audio:
        config_dict = _remap_omnistral_args(config_dict)
    elif is_vision:
...
def _remap_omnistral_args(config: dict) -> dict:
    # Handle combined vision and audio for Omnistral
    multimodal_config = config.get("multimodal", {})
    vision_config = multimodal_config.get("vision_encoder_args", {})
    ...
    # Build audio config from whisper args
