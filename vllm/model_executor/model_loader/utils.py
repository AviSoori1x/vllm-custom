...
def initialize_model(
    ...
    *,
    ...
    # ADD DEBUG HERE WITH CALL STACK
    import traceback
    print(f"🚀 DEBUG MODEL DETECTION: model_class = {model_class}")
    print(f"🚀 DEBUG MODEL DETECTION: model_class name = {model_class.__name__}")
    print(f"🚀 DEBUG MODEL DETECTION: architectures = {getattr(model_config.hf_config, 'architectures', 'NOT SET')}")
    print(f"🚀 DEBUG MODEL DETECTION: prefix = '{prefix}'")
    print(f"🚀 DEBUG MODEL DETECTION: Call stack:")
    for line in traceback.format_stack()[-3:-1]:  # Show last 2 calls
        print(f"  {line.strip()}")
